...
{
   puts(" ¬ведите новое количества товара (например: 12 кг или 15 шт)");
   scanf_kptovar(&M->kol);
} 
...
{
   puts(" ¬ведите новую цену товара (например: 15 $ или 180 руб)");
...
{
   puts(" ¬ведите новый срок реализации товара");
   scanf_date(&M->date);
}
...
{
   puts(" ¬ведите новых поставщиков товара");
    ...
    do
     {
       ...
       puts("     “овар успешно от корректирован!");   
       }     
      m=Vyxod(); 
    }
    while(m == 1);  
...
void poisk_kol(struct TOVAR *M, int B) //функци€ поиска товара по его количеству
{
    int i, n=0;
    struct kp_tovar A;
    printf(" ¬ведите количество товара (например: 12 кг или 15 шт)\n");
    scanf_kptovar(&A);
    for(i=0; i<B; i++)
...
void poisk_price(struct TOVAR *M, int B)  //функци€ поиска товара по его цене
{
    int i, n=0;
    struct kp_tovar A;
    printf(" ¬ведите цену товара (например: 12 $ или 180 руб)\n");
    scanf_kptovar(&A);    
    for(i=0; i<B; i++)
...
void poisk_sup(struct TOVAR *M, int B)  //функци€ поиска товара по его поставщику
{
    int j, i, n=0;
    char A[80];
    printf(" ¬ведите поставщика товара\n");
    fflush(stdin);
    gets(A);    
    for(i=0; i<B; i++)
...
void poisk_kps(struct TOVAR *M, int B)  //функци€ поиска товара по его количеству, цене и поставщику
 {
    ...
    printf(" ¬ведите количество товара (например: 12 кг или 15 шт)\n");
    scanf_kptovar(&a);
    printf(" ¬ведите цену товара за %s (например: 15 $ или 180 руб)\n", a.A);
    scanf_kptovar(&A);
    printf(" ¬ведите поставщика товара\n");
    fflush(stdin);
    gets(S);
